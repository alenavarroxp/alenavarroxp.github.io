  ...
  function comprobar(item: string) {
    switch (item) {
      ...
      case "CSS":
        return <img src={css} />;
      case "JavaScript":
        return <img src={javascript} />;
      case "Tailwind":
        return <img src={tailwind} />;
    }
    return <></>;
  }
  return (
    <div className="hidden lg:block md:block md:m-2 md:pr-4 md:pl-4 lg:p-0 lg:m-0 ">
      <Reorder.Item
        value={item}
        id={item}
        style={{ y, margin: 10, display: "flex", alignItems: "center" }}
      >
        <div className="mr-1 pointer-events-none object-none lg:w-8 md:w-8 h-auto">
          {comprobar(item)}
        </div>
